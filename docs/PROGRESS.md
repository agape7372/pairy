...
파운데이션 (감사 대응)  █████████████████░░░  85% — Tier 0 보안·런타임·결제 백엔드 완료. 실키 활성화·성능 남음
```
...
| 항목 | 내용 | 산출 |
|------|------|------|
| **결제 백엔드 (Tier 0 #6)** | 구독 1회성 결제 스캐폴드(Toss 테스트모드) — prepare/confirm/webhook Route Handler + payments 테이블 + grant_subscription. 금액 서버강제·멱등·부여 service_role만 (DL-0005) | (결제 PR) |

### 남은 파운데이션 (다음 우선순위)

| 항목 | 내용 | 선행 |
|------|------|------|
| 실 결제 활성화 | 상점 계약 후 Toss 실키 교체 + 웹훅 서명검증 강화. 지금은 테스트모드 | 사업자등록 |
| C-4 수익/구매 서버검증 | 마켓 판매·정산을 서버 원장으로. F-27 단건 구매는 payments 인프라 재사용 | 결제 백엔드(완료) |
